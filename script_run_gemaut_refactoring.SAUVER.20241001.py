...
def Decoupe_chantier(mns_file, masque_file, init_file, taille_dalle, iTailleRecouvrement, no_data_value, RepTravail_tmp, iNbreCPU):
    ...
    with rasterio.open(mns_file) as mns_src:
        ...
        print(f"pas_x >> {pas_x} pas_y >> {pas_y} NbreDalleX >> {NbreDalleX} NbreDalleY >> {NbreDalleY}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        cmd_masque4gemo="%s -i %s -e'I1.1==%d?0:255' -o %s -tuc " %(cmd_XinG,chemMASQUE,groundval,chemMASQUE_4gemo)
        print(cmd_masque4gemo)
        os.system(cmd_masque4gemo)

        # Affectation d'une valeur spécifique pour gérer les bords de chantier = 11 (no_data_interne_mask)
        cmd_masque_nodata="%s -i %s %s -e'I2.1==%d?%d:I1.1' -o %s -tuc " %(cmd_XinG,chemMASQUE_4gemo,chemMNS_IN,no_data_ext,no_data_interne_mask,chemMASQUE_nodata)
        print(cmd_masque_nodata)
        ...
        cmd_bouchage_trous="%s -i %s -FB:1:I:3:3:R%d -EM=%d,%d -o %s -n: " %(cmd_XinG,chemMNS_IN,no_data_int,no_data_ext,no_data_int,chemMNS)
        ...
        print("cmd_SousEch_MNS      >>> ",cmd_SousEch_MNS)
        ...
        print("cmd_SousEch_INIT >>> ",cmd_SousEch_INIT)  
        os.system(cmd_SousEch_INIT)     
        ...
        cmd_nodata=f"{cmdxingng} -i {chemMNT_OUT_tmp} {chemMNS_SousEch_tmp} -e'I2.1=={no_data_ext}?{no_data_ext}:I1.1' -o {chemMNT_OUT}"
        print(cmd_nodata)
        ...
        if args.clean: shutil.rmtree(RepTravail_tmp)
